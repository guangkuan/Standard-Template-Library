...
/*
/ 运用map，为的是能够根据键值迅速搜寻元素。这一点，不论是其底层是RB-tree或是hashtable，都可以达成任务。
/ 但是，RB-tree有自动排序功能而hashtable没有，反应出来的结果就是，map的元素有自动排序功能而hash_map没有。
/ hash_map的用法与map完全相同
...
template <class _Key, class _Tp, class _HashFcn  __STL_DEPENDENT_DEFAULT_TMPL(hash<_Key>), class _EqualKey __STL_DEPENDENT_DEFAULT_TMPL(equal_to<_Key>), class _Alloc =  __STL_DEFAULT_ALLOCATOR(_Tp) >
class hash_multimap;
